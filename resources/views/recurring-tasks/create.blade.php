<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('messages.recurring-tasks.Create Recurring Task') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('recurring-tasks.store') }}" x-data="{ frequency: '{{ old('frequency', '') }}', submitting: false }" @submit="submitting = true">
                        ...
                        <!-- Start Date -->
                        <div class="mt-4">
                            <x-input-label for="start_date" :value="__('messages.recurring-tasks.Start Date (Optional)')" />
                        ...
                        <!-- End Date -->
                        <div class="mt-4">
                            <x-input-label for="end_date" :value="__('messages.recurring-tasks.End Date (Optional)')" />
                        ...
                        <div class="flex items-center justify-end mt-6">
                            ...
                            <x-primary-button ::disabled="submitting">
                                <span x-show="!submitting">{{ __('messages.recurring-tasks.Create Recurring Task') }}</span>
                                <span x-show="submitting" x-cloak>{{ __('messages.Creating') }}</span>
                            </x-primary-button>
                        </div>
                    </form>
                </div>
